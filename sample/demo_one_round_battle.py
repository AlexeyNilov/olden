...
def run_demo_one_round_battle(
    ...
    turns_taken = _resolve_one_round(battle, combat_log, stack_ids)
    result = OneRoundBattleResult(
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Print a simplified one-round army battle calculation.")
    parser.add_argument(
        "attacker_army",
        nargs="?",
        type=Path,
    ...
    run_demo_one_round_battle(args.attacker_army, args.defender_army)
...
def _resolve_one_round(battle: Battle, combat_log: CombatLog, stack_ids: tuple[str, ...]) -> int:
...
def _print_result(result: OneRoundBattleResult, attacker_path: Path, defender_path: Path) -> None:
    print("Initial army matchup estimate")
    _print_estimate(result.initial_estimate, attacker_path, defender_path)
    print()
    print("Resulting state after one round")
